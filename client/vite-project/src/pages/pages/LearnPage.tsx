...
function LearnPage() {
    ...
    useEffect(() => {
        ...
        axios.get(API_USER_STATS_URL(authState.id)).then((response) => {
            if (response.data.error) {
                ...
                setUserCompletedCourses(
                    new Set(JSON.parse(completedLevels || '[]'))
                );
    ...
    useEffect(() => {
        if (allHanzi.length !== 0) {
            ...
            if (currentUserReview) {
                ...
                const filteredItems = currentUserReview.filter(
    ...
    const recurseHSKLevel = (currentLevel: number): number => {
        ...
        const filteredHSKLevelItems = allHanzi.filter(
            (item) => item.hskLevel == currentLevel
        );
    ...
    const startLearning = () => {
        ...
        if (allHanzi !== undefined) {
            for (let i = 0; i < allHanzi.length - 1; i++) {
                if (
                    !studiedHanzi.has(allHanzi[i].simplified) &&
        ...
        if (learningList.length > 0) {
            const sortedArray: HanziRow[] = learningList
                .slice()
                .sort((a, b) => a.id - b.id);
    ...
    const startReviewing = () => {
        ...
        const reviewSet = new Set(
            currentReviewItems.map((item) => item.material)
        );
                        ...
                        <div className="course-grid-wrapper">
                            <div
                                ...
                                {items.map((item) => {
                                    ...
                                    const prerequisitesArray = JSON.parse(
                                        item?.prerequisites || '[]'
                                    );
                                    const allPrerequisitesCompleted =
                                        prerequisitesArray.prerequisites.every(
                                            (prerequisite: number) =>
                                                userCompletedCourses.has(
                                                    prerequisite
                                                )
                                        );
