...
function HanziPage(HanziPageProps: any) {
    ...
    const handlePrevClick = () => {
      if (!isCooldown) {
        ...
        if (isLearning) {
          ...
          if (indexMinusOne >= 0) {
            navigate(`/hanzi/${ newIndex }`, { state: { isLearning: isLearning, learningList: learningList, learningIndexes: learningState.learningIndexes, currentIndex: indexMinusOne } });
      ...
      if (!isCooldown) {
        ...
        if (isLearning) {
          if (learningIndex + 1 < 5) {
            ...
            navigate(`/hanzi/${ newIndex }`, { state: { isLearning: isLearning, learningList: learningList, learningIndexes: learningState.learningIndexes, currentIndex: learningIndex + 1 } });
            startCooldown();
          } else {
            //review
            setIsAtReview(true);
    ...
    useEffect(() => {
      if (items.length > 0) {
        ...
        navigate(newUrl, { replace: true, state: { isLearning:isLearning, learningList: learningList, learningIndexes: learningState?.learningIndexes, currentIndex: learningIndex} });
        ...
        retrievedData.forEach((item: { sentence: any; translation: any; pinyin: any }) => {
          newSentencesMap.set(item.sentence, { translation: item.translation, pinyin: item.pinyin });
        });
    ...
    const updateRedirectionOption = (option: boolean | null) => {
      ...
      if (option === true) {
        navigate(`/hanzi/review`, { state: { isLearning: isLearning, learningList: learningList, learningIndexes: learningState.learningIndexes, currentIndex: learningIndex } });
      } else {
        setIsAtReview(false);
      }
    };
    ...
    return (
      <div className='hanzi-dictionary-wrapper'>
        <div className='hanzi-dictionary-main'>
          ...
          {isLearning && (
            <p className='bottom-right learning-progress'>{learningIndex + 1}/{maxLessonsBeforeTest?.toLocaleString()}</p>
          )}
          ...
          <h3 className='hanzi-dictionary-pinyin'>{items[0]?.pinyin} <TTSButton text={`${items[0]?.simplified}`}/></h3>
                  ...
                  <div className="hanzi-writer-container">
                    {items[0]?.simplified && 
                      (<div>
                        ...
                        <p><span>{items[0]?.simplifiedStrokeCount}</span> {EN_LC_STROKES_HEADER}</p>
                      </div>)}
                    {items[0]?.traditional && 
                      (<div>
                        <HanziWriterComponent character={items[0]?.traditional} />
                        <p><span>{items[0]?.traditionalStrokeCount}</span> {EN_LC_STROKES_HEADER}</p>
                      </div>)}
                  </div>
                </div>
              </div>
