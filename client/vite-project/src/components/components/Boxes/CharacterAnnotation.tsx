...
const CharacterAnnotation: React.FC<CharacterAnnotationProps> = ({ chineseWords }) => {
    ...
    const [userBookmarkedCharacters, setUserBookmarkedCharacters] = useState<Set<string>>();
        ...
        axios.get(`http://localhost:3001/api/texseg/cn/${chineseWords}`).then((response) => {
            if (response.data) {
                ...
                const updatedSegmentedWords = segmentedWords.flatMap((word: string) => {
                    const hanziInformation = allHanziContext.allHanziState.find((obj) => obj.simplified === word);
    ...
    const handleHover = (word: string, event: React.MouseEvent | React.TouchEvent) => {
        ...
        if (allHanzi) {
            ...
            if (hanziInformation) {
                ...
                meanings.push(validator.formatDefinition(hanziInformation.definition));
                ...
                const isBookmarked = userBookmarkedCharacters?.has(hanziInformation.simplified) ?? false;
        ...
        if (!isBookmarkCooldown) {
            ...
            if (annotation?.word[0]) {
                setUserBookmarkedCharacters((prevUserBookmarkedCharacters) => {
                    const newUserBookmarkedCharacters = new Set(prevUserBookmarkedCharacters || []);
